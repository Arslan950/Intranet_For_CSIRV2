    ...
    <div className="flex min-h-screen ">
      ...
      >
        ...
        <nav className="flex-1 mt-4 space-y-2 ">
          <Link  to={"/document"} className=" flex justify-start items-center py-2 px-3 hover:bg-gray-700 cursor-pointer">
            📁 <span className={`transition ease-in duration-500 ${open? "block":"hidden"}`} > {open && "Documents"}</span>
          </Link>
          <Link to={"/notice"} className="px-3 flex justify-start items-center py-2 hover:bg-gray-700 cursor-pointer">
            📌           <span className={`transition ease-in duration-500 ${open? "block":"hidden"}`} > {open && "Notices"}</span>

          </Link>
          <Link to={"/member"} className="px-3 flex justify-start items-center py-2 hover:bg-gray-700 cursor-pointer">
            👥 <span className={`transition ease-in duration-500 ${open? "block":"hidden"}`} >{open && "Members"}</span>
          </Link>
           <Link to={"/alerts"} className="px-3 flex justify-start items-center py-2 hover:bg-gray-700 cursor-pointer">
